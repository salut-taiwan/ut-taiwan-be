...
function generatePlaceholderModulesSQL(allPrograms) {
  const lines = [
    '-- ============================================================',
  ...
  // Collect unique modCode → name (first subject that references the code wins)
  const modCodeToName = new Map();
  for (const { subjects } of Object.values(allPrograms)) {
    for (const s of subjects) {
      for (const mc of s.moduleCodes) {
        if (!modCodeToName.has(mc)) {
          modCodeToName.set(mc, s.name);
        }
      }
    }
  }

  if (modCodeToName.size === 0) return lines.join('\n');

  // 1. Placeholder module inserts
  lines.push('-- ── Step 1: placeholder modules ─────────────────────────────────');
  for (const [mc, name] of modCodeToName) {
    lines.push(
      `INSERT INTO modules (tbo_code, name, is_available, price_student, price_general, publisher)` +
      ` VALUES (${esc(mc + '-1')}, ${esc(name)}, false, NULL, NULL, 'Universitas Terbuka')` +
  ...
  lines.push('-- ── Step 2: subject → module links ──────────────────────────────');
  for (const [dbCode, { subjects }] of Object.entries(allPrograms)) {
    const seenLinks = new Set();
    for (const s of subjects) {
      if (s.moduleCodes.length === 0) continue;
  ...
  // 3. Re-run package_modules links (idempotent — same as in 004)
  lines.push('-- ── Step 3: package → module links ──────────────────────────────');
  for (const [dbCode, { subjects }] of Object.entries(allPrograms)) {
    const semesters = [...new Set(subjects.map(s => s.semester_hint).filter(Boolean))].sort((a, b) => a - b);
    for (const sem of semesters) {
      lines.push(

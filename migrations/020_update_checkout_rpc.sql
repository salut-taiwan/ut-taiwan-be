-- Migration 020: Update checkout_order() to persist sku_id and variant_label for merchandise items
...
BEGIN
  ...
  -- Only block non-request module items that became unavailable; merch items (module_id IS NULL) are skipped
  SELECT m.tbo_code INTO v_unavailable
  FROM cart_items ci
  JOIN modules m ON m.id = ci.module_id
  WHERE ci.cart_id = v_cart_id
    AND ci.is_request = false
    AND ci.module_id IS NOT NULL
  ...
  INSERT INTO order_items (
    order_id, module_id, module_code, module_name,
    quantity, unit_price, subtotal, is_request, request_status,
    sku_id, variant_label
  )
  SELECT
    v_order.id,
    (item->>'module_id')::uuid,

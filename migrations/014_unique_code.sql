-- Migration 014: Add unique_code to payments for easy payment matching
ALTER TABLE payments ADD COLUMN IF NOT EXISTS unique_code INT DEFAULT 0;

-- Update checkout_order RPC to accept and store unique_code
...
BEGIN
  ...
  SELECT v_order.id,
    (item->>'module_id')::uuid, item->>'module_code', item->>'module_name',
    (item->>'quantity')::int, (item->>'unit_price')::numeric, (item->>'subtotal')::numeric
